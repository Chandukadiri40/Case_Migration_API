...
public class JobController {
    ...
    public ResponseEntity<List<MigrationJob>> getAllJobs(@RequestParam(required = false) String category) {
        ...
        if (category != null && !category.trim().isEmpty()) {
            jobs = jobRepository.findByCategoryOrderByCreatedAtDesc(category);
        } else {
        ...
        return jobRepository.findById(id).map(existing -> {
            if ("Completed".equalsIgnoreCase(existing.getStatus())) {
                ...
                boolean isChanged = !java.util.Objects.equals(existing.getFilterCriteria(), updatedJob.getFilterCriteria())
                        || !java.util.Objects.equals(existing.getConfiguration() != null ? existing.getConfiguration().getImportTarget() : null, updatedJob.getConfiguration() != null ? updatedJob.getConfiguration().getImportTarget() : null)
                        ...
                        || !java.util.Objects.equals(existing.getConfiguration() != null ? existing.getConfiguration().getDocIds() : null, updatedJob.getConfiguration() != null ? updatedJob.getConfiguration().getDocIds() : null)
                ...
                com.migrationreport.entity.JobAuditEntry audit = new com.migrationreport.entity.JobAuditEntry();
                audit.setModificationReason(updatedJob.getModificationReason());
                audit.setModifiedBy("admin");
                audit.setModifiedAt(LocalDateTime.now());
                audit.setChangesMade("Changed configuration from previous state. Target: " + (updatedJob.getConfiguration() != null ? updatedJob.getConfiguration().getImportTarget() : "null") + ", Criteria: " + updatedJob.getFilterCriteria());
                
                if (existing.getAuditHistory() == null) {
                    existing.setAuditHistory(new java.util.ArrayList<>());
                }
                existing.getAuditHistory().add(audit);
            ...
            existing.setConfiguration(updatedJob.getConfiguration());
    ...
    public ResponseEntity<MigrationJob> updateJobStatus(@PathVariable Long id, @RequestBody java.util.Map<String, Object> updates) {
        return jobRepository.findById(id).map(existing -> {
            ...
            if (updates.containsKey("failureReason")) existing.setFailureReason((String) updates.get("failureReason"));
